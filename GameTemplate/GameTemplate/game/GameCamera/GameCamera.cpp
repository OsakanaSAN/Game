...
void GameCamera::Strat()
{
	...
	Springcamera.Init(Eyepos, camera.GetLookatPt(),40000.0f); //バネカメラの初期化
	Springcamera.SetPosition(Eyepos);
	Springcamera.SetTarget (D3DXVECTOR3(0.0f, 0.0f, 0.0f));
...
void GameCamera::CameraAngle()
{
	...
	float lStick_x = pad.GetLStickXF();
		...
		/*if (LookOn)
		{
			...
			D3DXVECTOR3 PlayerBack =
			{
				game->GetPlayer()->GetWorldMatrix().m[2][0],
				game->GetPlayer()->GetWorldMatrix().m[2][1],
				game->GetPlayer()->GetWorldMatrix().m[2][2]
			};
			D3DXVec3Add(&Vpos, & Vpos,&(-PlayerBack * 12.0f));
