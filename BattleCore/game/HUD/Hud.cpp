...
void CHud::Start()
{
	...
	m_Map[0] = new Sprite;
	m_Map[0]->Loadtex("Assets/sprite/back_Hp.png");
	m_Map[0]->Initialize();
	
	m_Map[0]->Setcolor(1.0f, 1.0f, 1.0f, 1.0f);
	m_Map[0]->SetSize({ 95.0f,120.0f });
	m_Map[0]->Setposition({ 1001.0f, -9.0f });

	m_Map[1] = new Sprite;
	m_Map[1]->Loadtex("Assets/sprite/Minimap.png");
	m_Map[1]->Initialize();

	m_Map[1]->Setcolor(1.0f, 1.0f, 1.0f, 1.0f);
	m_Map[1]->SetSize({ 155.0f,150.0f });
	m_Map[1]->Setposition({ 1077.0f, 57.0f });
...
void CHud::Update()
{
	...
	m_Map[0]->SetTexture(game->GetPlayer()->GetMap()->GetTexture());
